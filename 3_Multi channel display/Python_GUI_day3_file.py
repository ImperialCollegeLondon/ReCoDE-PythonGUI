...
def create_window():
    ...
    with dpg.window(label="Window1", tag="win"):
        ...
        labels = "amplitude/mmHg", "amplitude/mV", "amplitude/mmHg"
        plot_tags = "y_axis1", "y_axis2", "y_axis3"

        # we use a for loop to create three plot widgets
        for i, (label, plot_tag) in enumerate(zip(labels, plot_tags)):
            with dpg.plot(label=label, height=200, width=600):
                ...
                dpg.add_plot_axis(dpg.mvYAxis, label="amplitude", tag=plot_tag)
