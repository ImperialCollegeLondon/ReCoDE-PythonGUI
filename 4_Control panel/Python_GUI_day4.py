...
# the symbol START_STOP is used to mark the start or end status
global START_STOP # 1:start 0:stop
# the symbol PAUSE is used to mark the pause status
global PAUSE # 1:pause 0:unpause
...
def start_GUI(ABP,ECG,ICP):
    ...
    # create the first window for display
    ...
    with dpg.window(label="Control", tag="win2", pos=[600, 0]):
        dpg.add_button(label="Start", callback=start)
        dpg.add_button(label="Pause", callback=pause)
        dpg.add_button(label="Stop", callback=stop)

    dpg.create_viewport(title='Multiple channel display', width=800, height=700)
    dpg.setup_dearpygui()
    dpg.show_viewport()
    i = 0
    """
